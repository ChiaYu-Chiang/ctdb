...
@login_required
@permission_required('ext_calendar.view_calendarevent', raise_exception=True, exception=Http404)
def event_list(request):
    template_name = 'ext_calendar' \
    '/calendar.html'
...
def calendar_events_json(request):
    ...
    for e in events:
        ...
        groups = ", ".join([g.name for g in e.support_group.all()])
        data.append({
            'id': e.id,
            'title': e.title,
            ...
            'extendedProps': {
                'groups': groups,
                'consultant': e.support_consultant,
...
@login_required
@permission_required('ext_calendar.add_calendarevent', raise_exception=True, exception=Http404)
def event_create(request):
    model = CalendarEvent
    instance = model(created_by=request.user)
    form_class = CalendarEventForm
    success_url = reverse('ext_calendar:event_list')
    form_buttons = ['create']
    template_name = 'ext_calendar/event_form.html'
    user_department = request.user.profile.activated_role
...
@login_required
@permission_required('ext_calendar.change_calendarevent', raise_exception=True, exception=Http404)
...
@login_required
@permission_required('ext_calendar.delete_calendarevent', raise_exception=True, exception=Http404)
def event_delete(request, pk):
    model = CalendarEvent
    event = get_object_or_404(model, pk=pk, created_by=request.user)
    success_url = reverse('ext_calendar:event_list')
    template_name = 'ext_calendar/event_confirm_delete.html'
    if request.method == 'POST':
        event.delete()
        return redirect(success_url)
    context = {'model': model}
...
def event_detail(request, pk):
    ...
    template_name = 'ext_calendar/event_detail.html'
    context = {'event': event}
    return render(request, template_name, context)